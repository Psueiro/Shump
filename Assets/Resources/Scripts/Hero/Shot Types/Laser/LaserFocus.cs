...
public class LaserFocus : IShotType
{
    List<Shot> _allShots;
    int _shotAmount;
    ...
    public LaserFocus(int shotAmount, float cooldown, float scale, float shotMaxTime, float distance)
    {
    ...
    public void Shoot()
    {
        Vector3 shotPos = new Vector3(_model.transform.position.x - (_allShots.Count * _distance) / 2, _model.transform.position.y);

        if (_model.focCooldown < _model.secMaxCooldown)
        {
    ...
    public void CallBullet(Vector3 pos, int startingNum)
    {
        for (int i = 0; i < _allShots.Count; i++)
        {
            ...
            _allShots[i].SetImperviousness(true).UnitDefiner(_model.myFocusShot);
